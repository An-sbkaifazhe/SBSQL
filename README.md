...
> 当"零信任"沦为营销热词的时代，SBSQL 展示了真正的零信任：
> **一个什么都不做、但做得完美的系统。**
...
5. 存活天数计算：自首次对外提供服务起，连续 **21 天**。
   ...
   **附则**：若能提供与本项目作者的**真实沟通记录**（Issue / 邮件 / IM 均可），
   经核实后，存活要求放宽至 `21 - 20 = 1` 天。
   即：**沟通记录 + 跑满 1 天**，即视为达成。

6. 期间不得偷偷换成 MySQL / PostgreSQL / SQLite / 任何"正经"数据库。
